    ...
    with open(output_csv_dir, mode='w') as file:
        ...
        if (obj_type == '10-K') | (obj_type == '10-Q'):
            writer.writerow(['web_url', 'nw_' + item_type, 'n_neg_lm_' + item_type, 'n_pos_lm_' + item_type,
                             'n_neg_gi_' + item_type, 'n_pos_gi_' + item_type, \
                             'n_neg_hr_' + item_type, 'n_pos_hr_' + item_type, 'n_uctt_lm_' + item_type,
                             'n_lit_lm_' + item_type, 'n_negation_lm_' + item_type, \
                             'n_negation_gi_' + item_type, 'n_negation_hr_' + item_type, 'fog_' + item_type,
...
def process_content(contents):
    ...
    # tokens =jit() word_tokenize(contents)
    word_tokenize_ = jit(List[str](str))(word_tokenize)

    tokens: List[str] = word_tokenize_(contents)
    ...
    words: List[str] = [w.lower() for w in tokens if w.isalpha() and len(w) > 1 or w == 'i']
...
for sublist in web_url:
    ...
    if (obj_type == '10-K') | (obj_type == '10-Q'):
        d = {'web_url': url_list, 'nw_' + item_type: nw, 'n_neg_lm_' + item_type: n_neg_lm,
             ...
             'n_neg_hr_' + item_type: n_neg_hr, 'n_pos_hr_' + item_type: n_pos_hr,
             ...
             'n_negation_hr_' + item_type: n_negation_hr, 'fog_' + item_type: fog, 'ari_' + item_type: ari,
             'smog_' + item_type: smog}
    else:
        d = {'web_url': url_list, 'nw': nw, 'n_neg_lm': n_neg_lm,
             'n_pos_lm': n_pos_lm, 'n_neg_gi': n_neg_gi, 'n_pos_gi': n_pos_gi, 'n_neg_hr': n_neg_hr,
             'n_pos_hr': n_pos_hr,
             'n_uctt_lm': n_uctt_lm, 'n_lit_lm': n_lit_lm, 'n_negation_lm': n_negation_lm,
             'n_negation_gi': n_negation_gi,
