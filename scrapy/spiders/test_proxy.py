...
def parse_html(html):  # 获取ip信息
    ...
    ips = soup.select('.table tbody tr')  # 返回列表 xpath
    for line in ips:
        ip = line.select_one('td').text  # http://49.70.89.49:9999
        port = line.select('td')[1].text
        print('[INFO]获取IP:{}  Port:{}'.format(ip, port))  # http://221.1.200.242:43399

        address = 'https://{}:{}'.format(ip, port)  # 构造ip访问
        proxies = {
            'http': address,  # http
            'https': address,  # https
        }
...
def verify_ip(proxies):  # 验证ip能否被用
    ...
    try:
        html = requests.get('http://www.baidu.com', proxies=proxies, timeout=3)  # 连接测试 超过3秒
