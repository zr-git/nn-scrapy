...
#     os.chdir(r'F:\github\narrative_conservatism\code')
os.chdir(r'C:\Users\Administrator\Desktop\zr_scrapy1\zr_scrapy1\spiders')
...
print(f'length of lm_neg:{len(lm_neg)}')
print(f'length of lm_pos:{len(lm_pos)}')
print(f'length of lm_uctt:{len(lm_uctt)}')
print(f'length of lm_lit:{len(lm_lit)}')
print(f'length of lm_cstr:{len(lm_cstr)}')
print(f'length of lm_modal1:{len(lm_modal1)}')
print(f'length of lm_modal2:{len(lm_modal2)}')
print(f'length of lm_modal3:{len(lm_modal3)}')
...
print(f'length of lm_stop:{len(lm_stop)}')
...
print(f'length of gi_neg:{len(gi_neg)}')
print(f'length of gi_pos:{len(gi_pos)}')
...
print(f'length of hr_neg:{len(hr_neg)}')
print(f'length of hr_pos:{len(hr_pos)}')
...
web_url=['https://www.sec.gov/Archives/edgar/data/1000045/000119312514034732/d669901d8k.htm']*10    #normal text
# web_url=['https://www.sec.gov/Archives/edgar/data/101063/000134100414000199/form8-k.htm']*10    #long text
# web_url=['http://english.sina.com/sports/2021-03-13/detail-ikknscsi3212699.shtml']*10
# web_url=[r'http://127.0.0.1/4.htm']*10

print(f'--------length of web_url:{len(web_url)}----------')
...
def get_neibor(item, l, len_of_l1):
    n = l.index(item)
    if n - 4 < 0:
        result = l[0:n + 4]
    elif n + 4 > len_of_l1:
        result = l[n - 4:len_of_l1]
    else:  # n - 4 > 0 and n + 4 < len(l):
        result = l[n - 4:n + 4]

    return result

def count_negation3(l1, l2, l3):
    count = 0
    len_of_l1=len(l1)
    s1 = set(l1)
    s2 = set(l2)
    ...
    for item in s:
        for item2 in get_neibor(item, l1, len_of_l1):
            if item2 in s3:
                count += 1
    return count
...
def process_data(contents):
    ...
    print(f'contents length:{len(contents)},tokens:{len(tokens)},words:{len(words)}')
...
if os.path.exists(output_csv_dir) == False:
    with open(output_csv_dir, mode='w') as file:
        ...
        if (obj_type == '10-K') | (obj_type == '10-Q'):
            writer.writerow(['web_url','nw_'+item_type, 'n_neg_lm_'+item_type, 'n_pos_lm_'+item_type,'n_neg_gi_'+item_type, 'n_pos_gi_'+item_type,                              'n_neg_hr_'+item_type, 'n_pos_hr_'+item_type, 'n_uctt_lm_'+item_type,'n_lit_lm_'+item_type, 'n_negation_lm_'+item_type,                              'n_negation_gi_'+item_type, 'n_negation_hr_'+item_type, 'fog_'+item_type, 'ari_'+item_type, 'smog_'+item_type])
        else:
            writer.writerow(['web_url','nw', 'n_neg_lm', 'n_pos_lm','n_neg_gi', 'n_pos_gi','n_neg_hr', 'n_pos_hr', 'n_uctt_lm','n_lit_lm',                              'n_negation_lm', 'n_negation_gi', 'n_negation_hr', 'fog', 'ari', 'smog'])
for sublist in web_url:
    ...
    if (obj_type == '10-K') | (obj_type == '10-Q'):
        d = {'web_url': url_list, 'nw_'+item_type: nw, 'n_neg_lm_'+item_type: n_neg_lm,              'n_pos_lm_'+item_type: n_pos_lm, 'n_neg_gi_'+item_type: n_neg_gi, 'n_pos_gi_'+item_type: n_pos_gi, 'n_neg_hr_'+item_type: n_neg_hr, 'n_pos_hr_'+item_type: n_pos_hr,              'n_uctt_lm_'+item_type: n_uctt_lm, 'n_lit_lm_'+item_type: n_lit_lm, 'n_negation_lm_'+item_type: n_negation_lm, 'n_negation_gi_'+item_type: n_negation_gi,              'n_negation_hr_'+item_type: n_negation_hr, 'fog_'+item_type: fog, 'ari_'+item_type: ari, 'smog_'+item_type: smog}
    else:
        d = {'web_url': url_list, 'nw': nw, 'n_neg_lm': n_neg_lm,              'n_pos_lm': n_pos_lm, 'n_neg_gi': n_neg_gi, 'n_pos_gi': n_pos_gi, 'n_neg_hr': n_neg_hr, 'n_pos_hr': n_pos_hr,              'n_uctt_lm': n_uctt_lm, 'n_lit_lm': n_lit_lm, 'n_negation_lm': n_negation_lm, 'n_negation_gi': n_negation_gi,              'n_negation_hr': n_negation_hr, 'fog': fog, 'ari': ari, 'smog': smog}
